...
def get_X_Y(Pc,seq_len=10,test_percent=0.2):
    ...
    fea_Pc = np.reshape(Pc, (Pc.shape[0], 1))
    ...
    #                        0,1,2              3         4       5        6      7                       8
    fea_total = np.hstack((fea_Pc,fea_BIAS6,fea_MA5,fea_ASY4,fea_yestPc))
    print 'yyhyyhyyh'
    ...
    y_train = seq_data[0:train_len,-1,0:1]
    x_test = seq_data[train_len::,0:-1,:]
    y_test = seq_data[train_len::,-1,0:1]
    # print 'yyhyyhyyhyhh'
    # print x_train
    # print 'stop'
...
if __name__=='__main__':
    ...
    for i in range(5):
        model = create_model([(4,4),50,100,1])#input_shape=[seq_len-1,indicator_num]
        if need_train == False:
            model.load_weights("my_model.h5")
        else:
            op = RMSprop(lr=0.0005)
            model.compile(loss="mean_squared_error", optimizer=op, metrics=['accuracy'])
            # model.fit(X_train, Y_train, nb_epoch=50, batch_size=150, verbose=1)
            early_stopping = EarlyStopping(monitor='val_loss', patience=2)
            model.fit(X_train, Y_train, validation_split=0.2, callbacks=[losshistory,early_stopping])
