...
def read_Pc(name):
    csvfile = file(name+'_total.csv', 'rb')
...
def get_X_Y(Pc,seq_len=10,test_percent=0.2):
    ranked_diff_series_3 = get_ranked_diff_3(Pc)
    Open=[]
    Close=[]
    High=[]
    Low=[]
    out = open('^gdaxi_total.csv')
    ...
    # print fey
    Open=np.array(Open)
    Open= np.reshape(Open, (Open.shape[0], 1))
    ...
    # print fey_binary_ranked_diff_2
    fea_binary_ranked_diff_3 = to_categorical(ranked_diff_series_3, 3)#5 dim
    ...
    Sy = SY(Pc)
    fea_ASY4 = ASY(3,Sy)#(4311,1)
    fea_ASY4 = np.reshape(fea_ASY4, (fea_ASY4.shape[0], 1))
    #print fea_binary_ranked_diff_5.shape#(4311,5)
    #                        0,1,2              3         4       5        6      7        8
    fea_total = np.hstack((fea_binary_ranked_diff_3,Open,Close,High,Low,fea_BIAS6,fea_MA5,fea_ASY4,fea_yestPc,fea_binary_ranked_diff_2))
...
if __name__=='__main__':
    ...
    X_train, X_test = select_fea(X_train,X_test,[0,1,2,3,4,5,6])
    scores = []
    for i in range(10):
        model = create_model([(4,7),100, 3])#input_shape=[seq_len-1,indicator_num]
        if need_train == False:
