...
def get_X_Y(Pc,seq_len=10,test_percent=0.2):
    ...
    #                        0,1,2              3         4       5        6      7        8
    fea_total = np.hstack((fea_binary_ranked_diff_3,fey_binary_ranked_diff_2,fea_BIAS6,fea_MA5,fea_ASY4,fea_yestPc,fea_binary_ranked_diff_2))
...
if __name__=='__main__':
    ...
    X_train, X_test = select_fea(X_train,X_test,[0,1,2])
    scores = []
    for i in range(10):
        model = create_model([(4,3),100, 3])#input_shape=[seq_len-1,indicator_num]
        if need_train == False:
