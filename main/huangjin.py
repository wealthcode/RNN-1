...
def get_X_Y(Pc,seq_len=10,test_percent=0.2):
    ...
    #                        0,1,2              3         4       5        6      7                       8
    fea_total = np.hstack((fea_binary_ranked_diff_3,fea_BIAS6,fea_changed,fea_MA5,fea_ASY4))
    print 'yyhyyhyyh'
...
if __name__=='__main__':
    ...
    for i in range(5):
        model = create_model([(4,5),50,100,3])#input_shape=[seq_len-1,indicator_num]
        if need_train == False:
            ...
            model.fit(X_train, Y_train, validation_split=0.2,nb_epoch=50, batch_size=150, callbacks=[losshistory])
